...
constexpr const char* kPluginsEnv = "ARCH_NAV_SUPERVISOR_PLUGINS";
constexpr const char* kSearchPathEnv = "ARCH_NAV_SUPERVISOR_PATH";
...
#ifdef ARCH_NAV_DEFAULT_SUPERVISOR_SEARCH_PATH
  paths.push_back(ARCH_NAV_DEFAULT_SUPERVISOR_SEARCH_PATH);
#endif
...
std::vector<std::string> collect_explicit_plugins() {
  return split_env_list(std::getenv(kPluginsEnv));
}
...
std::vector<std::string> list_plugins_in_directory(const std::string& directory) {
  ...
  if (dir == nullptr) return plugins;

  constexpr const char* kPrefix = "libarch_nav_supervisor_";
  ...
  for (dirent* entry = readdir(dir);; entry = readdir(dir)) {
    ...
    if (is_regular_file(path)) plugins.push_back(path);
  }
...
SupervisorPluginLoader::~SupervisorPluginLoader() {
  for (auto it = handles_.rbegin(); it != handles_.rend(); ++it) {
    dlclose(*it);
  }
}

void SupervisorPluginLoader::load_all() {
  ...
  for (const auto& path : collect_explicit_plugins()) {
    if (path.empty() || loaded_set.find(path) != loaded_set.end()) continue;
    load_library(path);
  ...
  for (const auto& directory : collect_search_paths()) {
    ...
    for (const auto& path : list_plugins_in_directory(directory)) {
      if (loaded_set.find(path) != loaded_set.end()) continue;
...
const std::vector<std::string>& SupervisorPluginLoader::loaded_plugins() const {
  return loaded_plugins_;
}

void SupervisorPluginLoader::load_library(const std::string& path) {
  void* handle = dlopen(path.c_str(), RTLD_NOW | RTLD_GLOBAL);
  if (handle == nullptr) {
    const char* error = dlerror();
    throw std::runtime_error(
        "Failed to load supervisor plugin '" + path + "': " +
